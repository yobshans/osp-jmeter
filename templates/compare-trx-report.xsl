...
<xsl:variable name="percent-fail-factor" select="0.20" />
<xsl:variable name="file2" select="document($testRes2)" />

<xsl:template match="test">
<html>
<head>
<title>Transaction Elapsed Time Report</title>
...
<xsl:template name="pageBody">
  ...
  <div class="body">
    <h3 align="center">Transaction Elapsed Time Report</h3>
    <hr size="1" />
	...
	<th>Difference (%)</th>
      ...
      <xsl:for-each select="/test/trx[contains(@label, 'Trx') and not(@label = preceding::*/@label)]">
