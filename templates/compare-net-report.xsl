...
<xsl:variable name="var_Monitor_Network" select="'Monitor_Network'" />
<xsl:variable name="percent-fail-factor" select="100" />
...
<body>
  <xsl:if test="$perfmonType = $var_Monitor_Network">
    <xsl:call-template name="pageBody" >
      <xsl:with-param name="title-label" select="'Network Utilization'" />
    </xsl:call-template>
  </xsl:if>
</body>
</html>
...
<xsl:template name="pageBody">
  ...
  <div class="body">
	...
	<th>Difference (Max)</th>
      ...
      <xsl:for-each select="/test/cpu[contains(@label, 'Network') and not(@label = preceding::*/@label)]">
	...
	<tr valign="top">
	  ...
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$minBase" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$minCur" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$diff-min" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$aveBase" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$aveCur" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$diff-ave" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$maxBase" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$maxCur" /></xsl:call-template></td>
	  <td align="center"><xsl:call-template name="display-net"><xsl:with-param name="value" select="$diff-max" /></xsl:call-template></td>
	</tr>
      </xsl:for-each>
    </table>
  </div>
...
<xsl:template name="display-net">
    <xsl:param name="value" />
    <xsl:value-of select="format-number($value,'#,###,###,### bytes/sec')" />
</xsl:template>
