...
<html>
<head>
<title>HTTP Requests Response Time Report</title>
...
<xsl:template name="pageHeader">
  ...
  <xsl:if test="$perfMon='true' or $perfMon='True'">
    <div class="topnav">
      ...
      <a class="active" href="api.html">HTTP Requests Response Time Report</a>
...
<xsl:template name="pageBody">
  ...
  <div class="body">
    <h3 align="center">HTTP Requests Response Time Report</h3>
    <hr size="1" />
    ...
    <!--xsl:for-each select="sample[contains(@lb, '.') and not(@lb = preceding::*/@lb)] or sample/sample[contains(@lb, '.') and not(@lb = preceding::*/@lb)]"-->
    <xsl:for-each select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx')) and not(contains(@lb, 'SSH')) and not(@lb = preceding::*/@lb)]">
