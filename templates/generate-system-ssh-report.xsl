...
<html>
<head>
<title>SSH Command Response Time Report</title>
...
<xsl:template name="pageHeader">
  ...
  <xsl:if test="$perfMon='true' or $perfMon='True'">
    <div class="topnav">
      ...
      <a class="active" href="ssh.html">SSH Commands Report</a>
...
<xsl:template name="pageBody">
  ...
  <div class="body">
    <h3>SSH Commands Response Time Report</h3>
    <hr size="1" />
    ...
    <xsl:for-each select="sample[contains(@lb, 'SSH') and not(contains(@lb, 'Trx')) and not(contains(@lb, 'setUp')) and not(@lb = preceding::*/@lb)]">
      ...
      <xsl:variable name="shortlabel" select="substring-before($label,':')" />
	...
	<td align="center">
	  <div class="popup" onclick="popupChart('{$shortlabel}')"><img width="25px" height="25px" src="../img/{$shortlabel}.png" />
	    <span class="popuptext"  id="{$shortlabel}" ><img src="../img/{$shortlabel}.png" /></span>
	  </div>
	</td>
	<td align="left"><xsl:value-of select="$shortlabel" /></td>
