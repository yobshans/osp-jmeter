...
<html>
<head>
<title>Openstack Web Horizon Functional Sanity Test Report</title>
...
<xsl:template name="pageHeader">
  ...
  <div class="topnav">
    ...
    <a href="html/api.html">HTTP Request/Response Data Report</a>
  </div>
...
<xsl:template name="pageBody">
  ...
  <xsl:variable name="allCount" select="count(sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))])" />
  <xsl:variable name="allFailureCount" select="count(sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))][attribute::s='false'])" />
  <xsl:variable name="allSuccessCount" select="count(sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))][attribute::s='true'])" />
  <xsl:variable name="allSuccessPercent" select="$allSuccessCount div $allCount" />
  <xsl:variable name="allTotalTime" select="sum(sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]/@t)" />
  <xsl:variable name="allAverageTime" select="$allTotalTime div $allCount" />
  <xsl:variable name="allMinTime">
    <xsl:call-template name="min"><xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]/@t" /></xsl:call-template>
  </xsl:variable>
  <xsl:variable name="allMaxTime">
    <xsl:call-template name="max"><xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]/@t" /></xsl:call-template>
  </xsl:variable>

  <xsl:variable name="slow1">
    <xsl:call-template name="maxtime">
      <xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]" /><xsl:with-param name="pos" select="1" />
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="slow2">
    <xsl:call-template name="maxtime">
      <xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]" /><xsl:with-param name="pos" select="2" />
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="slow3">
    <xsl:call-template name="maxtime">
      <xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]" /><xsl:with-param name="pos" select="3" />
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="slow4">
    <xsl:call-template name="maxtime">
      <xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]" /><xsl:with-param name="pos" select="4" />
    </xsl:call-template>
  </xsl:variable>
  <xsl:variable name="slow5">
    <xsl:call-template name="maxtime">
      <xsl:with-param name="nodes" select="sample[contains(@lb, '.') and not(contains(@lb, 'Trx'))]" /><xsl:with-param name="pos" select="5" />
    </xsl:call-template>
  </xsl:variable>
  ...
  <div class="body">
    <h3 align="center">Openstack Web Horizon Functional Sanity Test Report</h3>
    <hr size="1" />
	...
	<th align="left">Top 5 Worst HTTP Requests Response Time:</th>
      </tr>
      <tr>
	<td>
	  <svg height="300px" width="100%" >
	    ...
	    <text x="350" y="125" >PASS: <xsl:value-of select="$allSuccessCount" /></text>
	    <text x="25" y="250" >FAIL: <xsl:value-of select="$allFailureCount" /></text>
	  </svg>
	</td>
	<td>
